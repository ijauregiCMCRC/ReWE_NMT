...
class Trainer(object):
    ...
    def drop_checkpoint(self, opt, epoch, fields, valid_stats):
        ...
        if opt.emb_regularizer:
            real_generator_reg = (real_model.generator_regularizer.module
                              if isinstance(real_model.generator_regularizer, nn.DataParallel)
                              else real_model.generator_regularizer)

        model_state_dict = real_model.state_dict()

        if opt.emb_regularizer:
            model_state_dict = {k: v for k, v in model_state_dict.items()
                                if ('generator' or 'generator_regularizer') not in k}
            generator_state_dict = real_generator.state_dict()
            generator_regularizer_state_dict = real_generator_reg.state_dict()
            checkpoint = {
                'model': model_state_dict,
                'generator': generator_state_dict,
                'generator_regularizer': generator_regularizer_state_dict,
    ...
    def drop_checkpoint_mine(self, opt, epoch, fields, valid_stats,saved_model):
        ...
        if opt.ReWE:
            ...
            checkpoint = {
                'model': model_state_dict,
                'generator': generator_state_dict,
                'generator_regularizer': generator_ReWE_state_dict,
                'vocab': onmt.io.save_fields_to_vocab(fields),
                'opt': opt,
                'epoch': epoch,
                'optim': self.optim,
